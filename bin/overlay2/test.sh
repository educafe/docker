...
if [ -z $1 ]; then
	...
	do
		...
		do
			...
			for diffdir in "$DOCKERHOME/image/overlay2/layerdb/sha256/"*
			do
				# echo $(basename ${diffdir#sha256:})
				diff_val=$(cat $diffdir/diff)
				diff_val=${diff_val#sha256:}
				if [ "$diffid" = "$diff_val" ]; then
					echo " Diffid: $diffid"
					echo "  Layerdb: "$(basename ${diffdir#sha256:})
				fi
			done
		done
