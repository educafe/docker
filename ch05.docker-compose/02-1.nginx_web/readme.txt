docker image build . -t app (app 디렉토리에서 실행)
docker image build . -t nginx (nginx 디렉토리에서 실행)
docker network create mynet
docker container run -d --name myapp --network mynet --restart=always app
docker container run -d --name mynginx --network mynet --restart=always -p 80:80 nginx
